 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a few numbers") {
    ...
    assert(sum(List(-7)) === -7)
    assert(sum(List(-1,-3,-10,9)) === -5)
  }

  test("max of a few numbers") {
    assert(max(List(3, 7, 2)) === 7)
    assert(max(List(0)) === 0)
    assert(max(List(11)) === 11)
    assert(max(List(-32)) === -32)
    assert(max(List(5,9)) === 9)
    assert(max(List(110,99)) === 110)
    assert(max(List(23,-34,9,-23,5)) === 23)
  }

  test("max throws exception") {
